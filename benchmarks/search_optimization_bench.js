...
function generateOriginal(q, cols) {
    ...
    // Mimic the original logic in src/stores/dataStore.ts
...
function generateOptimized(q, cols) {
    ...
    // Optimized: CONCAT_WS
    // We cast each column to VARCHAR just in case, though concat_ws handles many types.
    // However, to be safe and consistent with previous behavior (casting everything), we keep explicit casts or rely on implicit.
    // DuckDB concat_ws implicitly casts.
    // But original code used explicit CAST. Let's try implicit cast first as it's cleaner, but explicit is safer for some types.
    // Actually, explicit CAST is better to avoid ambiguity.
...
const scenarios = [
  ...
  { name: "Double Quote Column", query: "foo" } // Should include the column with quotes
];

console.log("--- Search Query Optimization Test ---");
...
scenarios.forEach(s => {
  ...
  // Count operations (approximate by counting keywords)
